...
const SummarizeSchema = z.object({
  summary: z
    .string()
    .describe(
      "A 3 to 6 sentence plain-text summary of the meeting, capturing key discussion points"
    ),
  followUpEmail: z.object({
    ...
    body: z
      .string()
      .describe(
        "Approachable and professional follow-up email text, listing key decisions and action items with owners and deadlines"
      ),
  }),
});
...
const generateSummaryAndEmail = async (rawTranscript, participants, actionItems, keyDecisions) => {
  ...
  const prompt = ChatPromptTemplate.fromMessages([
    [
      "system",
      "You are an expert AI meeting assistant. Your task is to write a meeting summary and draft a follow-up email based on the transcript and already-extracted meeting elements. Follow this schema:\n{schema_instructions}",
    ],
    [
      "human",
      "Meeting Transcript:\n{transcript}\n\nParticipants:\n{participants}\n\nAction Items:\n{action_items}\n\nKey Decisions:\n{decisions}",
  ...
  try {
    ...
    const repairPrompt = ChatPromptTemplate.fromMessages([
      [
        "system",
        "You are an expert AI meeting assistant. Your previously generated summary output contains schema validation errors. Correct it. Output ONLY valid raw JSON matching the schema.\n{schema_instructions}",
      ],
